namespace MqttStressTester.Tests
{
    ...
    public abstract class TestBase
    {
        ...
        {
            var result = this.Client.Connect(ClientId.ToString(), ClientId.ToString(), string.Empty, true, 30);
            if (result != 0)
            {
        ...
        {
            var v = this.Client.Publish(topic, Encoding.UTF8.GetBytes(message), MqttMsgBase.QOS_LEVEL_AT_MOST_ONCE, false);
        }
